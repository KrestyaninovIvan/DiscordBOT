...
async def инфо(ctx, arg=None):
    ...
    if arg is None:
        ...
        text = '\n!test - Бот онлайн?\n !Статус - мои матюки ' \
               '\n!статистика пользователь @имя период(в днях,НЕ ОБЯЗАТЕЛЬНО) - вывод информации ' \
               'по играм пользователя ' \
               '\n!статистика пользователи - вывод информации времени в играх всех пользователей ' \
               '\n!статистика я период(в днях,НЕ ОБЯЗАТЕЛЬНО) - вывод информации себя' \
               '\n!топ - вывод топ по времени в игре'
        await ctx.send(f'{author.mention,text} ')
